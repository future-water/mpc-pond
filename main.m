...
%cin_f = [sum(data1.qin.*data1.cin)/sum(data1.qin)*ones(577,1); zeros(500,1)]; % Imperfect water quality prediction as EMC
...
for k = 1:(length(MD)-horizon-1)
    ...
    yk = pondcstr_StateFcn(mpc_x(k,:), [mpcekf.MVopt(2,:), MD_t(k,:)]);
...
for k = 1:(length(MD)-horizon-1)
    yk = pondcstr_StateFcn(mpc_x(k,:), [mpcekf.MVopt(2,:), MD_t(k,:)]);
    random = [0, 0.05].*randn(1,2).*mpcekf.Xopt(2,:);
    yk = max(0, yk'+random);
    xxk = correct(EKF, yk(2));
    xxk = [mpcekf.Xopt(2,1), xxk(2)];
    [~,~,mpcekf] = nlmpcmove(nlmpcobj_Plan,xxk, mpcekf.MVopt(2,:),yref,MD(k+1:(horizon+k),:));
...
% cumulative load
load = [max(cumsum(truec.*mpc_ekfy)*10^(-3)*15*60), max(cumsum(C_c.*[0,q_out_c]+spill_c)*10^(-3)*15*60), max(cumsum(C_q.*[0,q_out_q]+spill_q)*10^(-3)*15*60), max(cumsum(nc.Xopt(1:T,2).*nc.Yopt(1:T,3))*10^(-3)*15*60)];
